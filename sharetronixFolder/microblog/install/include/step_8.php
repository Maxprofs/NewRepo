	...
	if( !isset($s['INSTALLED']) || !$s['INSTALLED'] )
	{
		...
		if( ! $error ) {
			...
			if( ! $error ) {
				...
				$config	= config_replace_variable( $config,	'$C->FACEBOOK_API_ID',			isset($OLDC->FACEBOOK_API_ID) ? $OLDC->FACEBOOK_API_ID : ''	 );
				$config	= config_replace_variable( $config,	'$C->FACEBOOK_API_SECRET',			isset($OLDC->FACEBOOK_API_SECRET) ? $OLDC->FACEBOOK_API_SECRET : ''	 );
				...
				$config	= config_replace_variable( $config,	'$C->GOOGLE_CAPTCHA_PRIVATE_KEY',		isset($OLDC->GOOGLE_CAPTCHA_PRIVATE_KEY) ? $OLDC->GOOGLE_CAPTCHA_PRIVATE_KEY : ''	 );
				$config	= config_replace_variable( $config,	'$C->GOOGLE_CAPTCHA_PUBLIC_KEY',		isset($OLDC->GOOGLE_CAPTCHA_PUBLIC_KEY) ? $OLDC->GOOGLE_CAPTCHA_PUBLIC_KEY : ''	 );
				$config	= config_replace_variable( $config,	'$C->FOOTER_REMOVE_BACKLINK',			isset($OLDC->FOOTER_REMOVE_BACKLINK)&&$OLDC->FOOTER_REMOVE_BACKLINK ? 'TRUE' : 'FALSE', FALSE );
				$config	= config_replace_variable( $config,	'$C->CRONJOB_IS_INSTALLED',			isset($OLDC->CRONJOB_IS_INSTALLED)&&$OLDC->CRONJOB_IS_INSTALLED ? 'TRUE' : 'FALSE', FALSE );
		...
		if( ! $error ) {
			if( $convert_version && $convert_version < '1.4.0' && $convert_version != 'unofficial' ) {
				...
				if( ! $error ) {
					...
					$tmp	.= "\t\t'name'\t\t=> 'Sharetronix ".date('Y-m-d H:i')."',\n";
					$tmp	.= "\t\t'version'\t\t=> '',\n";
					$tmp	.= "\t\t'image'\t\t=> '',\n";
					$tmp	.= "\t\t'description'\t=> 'This theme was automatically generated on the Sharetronix upgrade process (for keeping old changes). It will now support some of the new Sharetronix features. If you don\'t need this theme, please delete it from the Themes folder.',\n";
					$tmp	.= "\t\t\n";
					$tmp	.= "\t\t'author_name'\t=> 'Sharetronix',\n";
					$tmp	.= "\t\t'author_url'\t=> 'http://sharetronix.com',\n";
					$tmp	.= "\t\t'author_email'\t=> 'sharetronix@blogtronix.com',\n";
					$tmp	.= "\t\t\n";
					$tmp	.= "\t\t'logo_height'\t=> '34',\n";
