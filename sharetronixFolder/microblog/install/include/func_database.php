<?php
	...
	function create_database($convert_version=FALSE)
	{
		...
		if( $v && $v < '2.2.1' ) {
			my_mysql_query('ALTER TABLE  `users` DROP  `used_storage`', $conn);
			my_mysql_query('ALTER TABLE  `users` DROP  `comments_expanded`', $conn);
			my_mysql_query('ALTER TABLE  `users` DROP  `dbrd_whattodo_closed`', $conn);
			my_mysql_query('ALTER TABLE  `users` DROP  `dbrd_groups_closed`', $conn);
			my_mysql_query('ALTER TABLE  `users` DROP  `js_animations`', $conn);
		}
		...
		if( $v && $v < '2.2.0' ) {
			my_mysql_query('CREATE TABLE  `notifications` (
			...
			my_mysql_query('ALTER TABLE  `users_dashboard_tabs` CHANGE  `tab`  `tab` ENUM(  \'\',  \'all\',  \'@me\',  \'private\',  \'commented\',  \'feeds\',  \'tweets\',  \'notifications\' ) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL', $conn);
			my_mysql_query('ALTER TABLE  `users_notif_rules` CHANGE  `ntf_me_on_post_like`  `ntf_me_on_post_like` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT  \'0\' COMMENT  \'0-off, 1-both, 2-post,  3-email\'', $conn);
			
			my_mysql_query('CREATE TABLE  `group_notifications` (
						`id` INT( 10 ) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY ,
			...
			my_mysql_query('ALTER TABLE `post_likes` ADD  `comment_id` INT( 5 ) UNSIGNED NOT NULL DEFAULT  \'0\' AFTER  `post_id`', $conn);
			my_mysql_query('ALTER TABLE `posts_comments` ADD  `likes` INT( 5 ) UNSIGNED NOT NULL DEFAULT  \'0\' AFTER  `mentioned`', $conn);
			my_mysql_query('ALTER TABLE `users_notif_rules` ADD `ntf_me_on_comment_like` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT \'0\' COMMENT \'0-off, 1-both, 2-post, 3-email\'', $conn);
		}
		if( $v == '2.1.0' ) {
			return TRUE;
		}
		...
		", $conn);
		$res	= $res && my_mysql_query("
			DROP TABLE IF EXISTS `".$prefix."group_notifications`;
		", $conn);
		$res	= $res && my_mysql_query("
			CREATE TABLE  `".$prefix."group_notifications` (
			`id` INT( 10 ) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY ,
			`notif_type` VARCHAR( 50 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL ,
			`to_group_id` INT( 5 ) UNSIGNED NOT NULL ,
			`from_user_id` INT( 8 ) UNSIGNED NOT NULL ,
