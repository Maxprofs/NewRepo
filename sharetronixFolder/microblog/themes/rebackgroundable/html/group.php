			...
			<div id="profile2">
				<div id="profile_left">
					...
					<div class="ttl" style="margin-bottom:8px;">
						<div class="ttl2">
							<h3><?= $this->lang('group_latest_members') ?></h3>
						</div>
					</div>
					...
					<div class="slimusergroup">
						...
						<div>
							...
							<span class="latest_group_joiners"> <?= $this->lang('group_latest_members_joined') ?> <br /> <strong><?= post::parse_date($u['date']) ?></strong> </span>
						</div>
						<?php } ?>
					</div>
					<?php } ?>
				...
				<div id="profile_right">
					<div id="profilehdr">
						...
						<div id="usermenu">
							<a href="javascript:;" onclick="<?= ($C->SPAM_CONTROL)? 'spam_control':'postform_open' ?>(({groupname:'<?= $D->g->title ?>'}));" class="um_ptg" onmouseover="userpage_top_tooltip(this.firstChild.innerHTML);" onmouseout="userpage_top_tooltip('');"><b><?= $this->lang('grp_toplnks_post',array('#GROUP#'=>$D->g->title)) ?></b></a>
							<?php if( ! $D->i_am_member ) { ?>
				...
				<?php } elseif( $D->tab == 'settings' ) { ?>
					...
					<div class="htabs" style="margin-bottom:6px;">
						...
						<a href="<?= userlink($D->g->groupname) ?>/tab:settings/subtab:rssfeeds" class="<?= $D->subtab=='rssfeeds'?'onhtab':'' ?>"><b><?= $this->lang('group_sett_subtabs_rssfeeds') ?> </b></a>
						<?php } ?>
						<a href="<?= userlink($D->g->groupname) ?>/tab:settings/subtab:admins" class="<?= $D->subtab=='admins'?'onhtab':'' ?>"><b><?= $this->lang('group_sett_subtabs_admins') ?> </b></a>
