...
?>
		<div id="profile">
			...
			<div id="profile_text">
				...
				<div id="profileicons">
					...
					<a href="<?= $C->SITE_URL.$D->usr->username ?>/show:info" class="pri_qr"></a>
				</div>
				<?php } ?>
			</div>
		</div>
		<div id="profilenav">
			...
			<?php if($D->he_follows_me || !$D->profile_protected){ ?>
		...
		<?php if( $D->show == 'updates' ) { ?>
			...
			<div id="postspage" style="border-top:0px;">
				<div id="posts">
					<?php if( $D->num_results == 0  && (!$D->posts_protected || ($this->user->is_logged && $this->user->info->is_network_admin)) ) { ?>
						<div class="alert yellow"><?= $this->lang('iphone_user_no_posts', array('#USER#'=>$D->usr->username)) ?></div>
					<?php } elseif($D->num_results == 0) { ?>
						<div class="alert yellow"><?= $this->lang('iphone_user_posts_protected', array('#USER#'=>$D->usr->username)) ?></div>
					<?php } else { ?>
						<?= $D->posts_html ?>
					<?php } ?>
				</div>
		...
		<?php } elseif( $D->show == 'info' ) { ?>
			...
			<div id="infopage">
				<b>QR</b>
				<div id="aboutme">
					<img src="<?= $D->qr->get_link(250) ?>" border='0' />
				</div>
				...
				<div id="infopagepanel">
					...
					<?php if( !empty($D->i->personal_email) && $D->usr->email!=$D->i->personal_email && $D->he_follows_me ) { ?>
