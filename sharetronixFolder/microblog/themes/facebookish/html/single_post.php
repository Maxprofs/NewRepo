		...
		<?php } else { ?>
			...
			<?php } ?>
				...
				<div class="postcontrols">
					...
					<?php } else { ?>
						<?php if( $D->p->if_can_edit() || $D->p->if_can_delete() || $D->p->if_can_reshare() || !$D->p->post_group || $D->p->post_group->is_public ) { ?>
							...
							<div class="pctrls" style="display:none;" id="dropcontrols_box_<?= $D->p->post_tmp_id ?>">
								...
								<div class="pctrls_sharediv">
									...
									<div class="pctrls_share_services" style="display:none;" id="dropcontrols_sharebox_<?= $D->p->post_tmp_id ?>">
										<?= $D->p->show_share_link() ?>
									</div>
								</div>
								<?php } ?>
						...
						<?php if( $D->p->post_type=='public' ) { ?>
							...
							<a href="javascript:;" id="postlink_unfave_<?= $D->p->post_tmp_id ?>" class="pfave saved" title="<?= $this->lang('post_unfave_link') ?>" style="<?= $D->p->is_post_faved()?'display:block;':'display:none;' ?>" onfocus="this.blur();" onclick="post_unfave('<?= $D->p->post_tmp_id ?>',<?= $this->request[0]=='dashboard'&&isset($D->tab)&&$D->tab=='bookmarks' ? 'true' :'false' ?>);"></a>
						<?php } ?>
					<?php } ?>
				</div>
				<div class="postbody">
				...
				<?php 
					$display = $this->param('opencomments')? 'block' : 'none';
					if(isset($D->display_comments) && $D->display_comments){
						$display = 'block';  
					}
				?>
				<div class="postcomments" id="postcomments_<?= $D->p->post_tmp_id ?>" style="display:<?= $display ?>;">
					<?php if( $D->p->post_commentsnum == 0 ) { ?>
					...
					<div class="postcommentscontent">
						...
						<div class="comment<?= $i==0?' firstcomment':'' ?>" id="postcomment_<?= $c->comment_id ?>">
							...
							<div class="comment_right">
                                                                <?php if( $c->if_can_delete() ) { ?>
                                                                <div class="onclosecomments">
                                                                    <a onclick="postcomment_delete('<?= $c->post->post_tmp_id ?>', <?= $c->comment_id ?>, '<?= $this->lang('post_delcomment_cnfrm') ?>');" href="javascript:;" onfocus="this.blur();" class="closecomments" title="<?= $this->lang('post_delcomment_lnk') ?>"></a>
                                                                </div>
                                                                <?php } ?>
								...
								<?= post::parse_api($c->comment_api_id) ?>&middot
							</div>
                                                        <?php if( $c->if_can_delete() ) { ?>
                                                        </div>
                                                        <?php } ?>
