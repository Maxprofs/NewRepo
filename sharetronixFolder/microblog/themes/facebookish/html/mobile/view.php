...
?>
	<?php if( $D->msg_reshared ) { ?>
	<hr />
		<div class="okbox" style="margin-top:10px;"><?= $this->lang('reshare_done') ?></div>
	<hr />
	<?php } ?>
			...
			<?php 
				if( $this->user->is_logged && $D->p->post_resharesnum>0 ) {
					...
					if( count(array_intersect($reshared_by,$i_follow))>0 ) {
						?>
						&middot; <?= $this->lang($D->p->post_resharesnum==1?'reshared_by_one':'reshared_by_some',array('#NUM#'=>$D->p->post_resharesnum)) ?>
						<?php
					}
				}
			?>
			<?php if( $D->p->if_can_reshare() ) { ?>
			&middot; <a href="<?= $D->p->permalink ?>?reshare=1" onclick="return confirm('<?= $this->lang('reshare_confirm') ?>');" onfocus="this.blur();"><?= $this->lang('reshare') ?></a>
			<?php } ?>
		</div>
	</div>
	<hr />
