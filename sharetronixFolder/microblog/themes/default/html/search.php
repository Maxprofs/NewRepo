...
?>
					<div id="invcenter">
						...
						<div class="htabs" style="margin-bottom:6px; margin-top:0px;">
							...
							<a href="<?= $C->SITE_URL ?>search/tab:tags/s:<?= urlencode($D->search_string) ?>" class="<?= $D->tab=='tags'?'onhtab':'' ?>"><b>Tags</b></a>
...
<?php } elseif($D->tab == 'posts') { ?>
					<div id="invcenter">
						...
						<div id="searchbostsleft">
							<div class="htabs" style="margin-bottom:6px; margin-top:0px;">
								<a href="<?= $C->SITE_URL ?>search/tab:posts/s:<?= urlencode($D->search_string) ?>" class="<?= $D->tab=='posts'?'onhtab':'' ?>"><b><?= $this->lang('srch_tab_posts') ?></b></a>
								<a href="<?= $C->SITE_URL ?>search/tab:tags/s:<?= urlencode($D->search_string) ?>" class="<?= $D->tab=='tags'?'onhtab':'' ?>"><b><?= $this->lang('srch_tab_tags') ?></b></a>
								<a href="<?= $C->SITE_URL ?>search/tab:users/s:<?= urlencode($D->search_string) ?>" class="<?= $D->tab=='users'?'onhtab':'' ?>"><b><?= $this->lang('srch_tab_users') ?></b></a>
								<a href="<?= $C->SITE_URL ?>search/tab:groups/s:<?= urlencode($D->search_string) ?>" class="<?= $D->tab=='groups'?'onhtab':'' ?>"><b><?= $this->lang('srch_tab_groups') ?></b></a>
							</div>
						...
						<div id="searchbostsright">
							...
							<form method="post" name="psrch" action="<?= $C->SITE_URL ?>search">
								...
								<div class="greygrad" id="srchposts_dropbox_type" style="display:block;' ?>;">
									<div class="greygrad2">
										<div class="greygrad3">				
											<label><input type="checkbox" name="ptype[]" value="link" <?= in_array('link', $D->ptypes_choosen)? 'checked="checked"':'' ?> /><span><?= $this->lang('srch_posts_ptp_link') ?></span></label>
											<label><input type="checkbox" name="ptype[]" value="image" <?= in_array('image', $D->ptypes_choosen)?'checked="checked"':'' ?> /><span><?= $this->lang('srch_posts_ptp_image') ?></span></label>
											<label><input type="checkbox" name="ptype[]" value="video" <?= in_array('video', $D->ptypes_choosen)?'checked="checked"':'' ?> /><span><?= $this->lang('srch_posts_ptp_video') ?></span></label>
											<label><input type="checkbox" name="ptype[]" value="file" <?= in_array('file', $D->ptypes_choosen)?'checked="checked"':'' ?> /><span><?= $this->lang('srch_posts_ptp_file') ?></span></label>
											<label><input type="checkbox" name="ptype[]" value="comments" <?= in_array('comments', $D->ptypes_choosen)?'checked="checked"':'' ?> /><span><?= $this->lang('srch_posts_ptp_comments') ?></span></label>
											<label><input type="checkbox" name="ptype[]" value="rss" <?= in_array('rss', $D->ptypes_choosen)?'checked="checked"':'' ?> /><span><?= $this->lang('search_without_type') ?> <?= $this->lang('srch_posts_ptp_rss') ?></span></label>
											<label><input type="checkbox" name="ptype[]" value="tweets" <?= in_array('tweets', $D->ptypes_choosen)?'checked="checked"':'' ?> /><span><?= $this->lang('search_without_type') ?>  <?= $this->lang('srch_posts_ptp_tweets') ?></span></label>
										</div>
									</div>
								</div>
...
?>
					<div id="invcenter">
						...
						<div id="searchbostsright">
							...
							<div class="ttl" style="margin-bottom:5px;"><div class="ttl2"><h3>Tags</h3></div></div>
							<div class="taglist">
								<?php foreach($D->post_tags as $tmp) { ?>
								<a href="<?= $C->SITE_URL ?>search/posttag:<?= $tmp ?>" class="<?= $tmp==$D->search_string?'ontag':'' ?>"><?= htmlspecialchars(str_cut($tmp,23)) ?></a>
								<?php } ?>
							</div>
							<?php } ?>
							...
							<div class="taglist">
								<?php foreach($D->saved_searches as $id=>$tmp) { ?>
								<a href="<?= $C->SITE_URL ?>search/saved:<?= $tmp->search_key ?>" class="<?= (isset($D->search_saved) && $id==$D->search_saved)?'ontag':'' ?>"><?= preg_replace('/^\#/', '<small>#</small>', htmlspecialchars(str_cut($tmp->search_string,23))) ?></a>
								<?php } ?>
							</div>
							<?php } ?>
						</div>
