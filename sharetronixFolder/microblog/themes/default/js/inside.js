...
var dbrd_tabs_to_check	= ["all", "@me", "private", "commented", "feeds", "tweets"];
...
function dbrd_check_tabs()
{
	...
	req.onreadystatechange	= function() {
		...
		dbrd_tabs_timeout	= setTimeout( dbrd_check_tabs, 180000 );
	};
...
var last_hdrmenu_tmout		= false;
function hdrmenu_dropsub(tmpid)
{
	if( ! pf_htmlobjects_shown ) {
		pf_htmlobjects_shown	= {};
	}
	if( pf_htmlobjects_shown["hdr_drop_"+tmpid] == undefined ) {
		pf_htmlobjects_shown["hdr_drop_"+tmpid]	= 0;
	}
	if( pf_htmlobjects_shown["hdr_drop_"+tmpid] != 0 ) {
	}
	hdrmenu_hidesub2();
	var bx2	= d.getElementById("hdr_drop_"+tmpid);
	if( ! bx2 ) {
		return false;
	}
	bx2.onmouseover	= function() { hdrmenu_keepdropped(); };
	bx2.onmouseout	= function() { hdrmenu_hidesub(tmpid); };
	var i, lnks	= bx2.getElementsByTagName("A");
	for(i=0; i<lnks.length; i++) {
		lnks[i].onmouseover	= function() { hdrmenu_keepdropped(); };
	}
	if( disable_animations ) {
	...
	last_hdrmenu_dropped	= bx2;
}
function hdrmenu_hidesub(tmpid)
{
	if( last_hdrmenu_tmout ) {
		clearTimeout( last_hdrmenu_tmout );
	}
	var bx2	= d.getElementById("hdr_drop_"+tmpid);
	last_hdrmenu_tmout	= setTimeout( function() {
		if( disable_animations ) {
			bx2.style.display	= "none";
		...
		last_hdrmenu_dropped	= false;
		last_hdrmenu_tmout	= false;
	}, 300);
}
function hdrmenu_hidesub2()
{
	if( ! pf_htmlobjects_shown ) {
		pf_htmlobjects_shown	= {};
	}
	if( last_hdrmenu_dropped ) {
		last_hdrmenu_dropped.style.display	= "none";
		pf_htmlobjects_shown[last_hdrmenu_dropped.id]	= 0;
		last_hdrmenu_dropped	= false;
	}
	if( last_hdrmenu_tmout ) {
		clearTimeout( last_hdrmenu_tmout );
...
function show_hide_div_by_id(div_id)
{
	...
	if(dv.style.display == "undefined" || dv.style.display == "none" || dv.style.display == "" ){
		dv.style.display = "block";
	}else{
		dv.style.display = "none";
	}	
