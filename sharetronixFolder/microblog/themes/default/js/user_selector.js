...
UserSelector.prototype.init	= function()
{
	...
	lnk1.href	= siteurl+"signup/follow?pg=1";
	...
	this.buffer = this.data; //added
	d.getElementById("userselectortab_sel_num").innerHTML	= this.selnum;
	obj.perpg = obj.perpg+this.selnum;//added
	setTimeout( function() { 
		obj.tab(obj.seltab);
		obj.onload();
	}, 1);
...
UserSelector.prototype.initdiv	= function(indx)
{
	...
	dv.onclick	= function() { 
		...
		req.open("POST", siteurl+"ajax/followusronreg/r:"+Math.round(Math.random()*1000), true);
		req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		req.send("uid="+encodeURIComponent(c[0])+"&type="+regime);
		//added end
	};
...
UserSelector.prototype.tab	= function(tb)
{
	...
	if(tb == 'sel'){
		d.getElementById("userselector_searchinp").style.display = 'none';		
	}else{
		d.getElementById("userselector_searchinp").style.display = 'block';			
	}
	this.seltab	= tb;
	this.searchi();
};
UserSelector.prototype.search	= function(txt)
{
	...
	req.onreadystatechange	= function() {
		if( req.readyState != 4  ) { return; }
		if( req.responseText == "ERROR" ){ return; }
		
		var srch_result = eval(req.responseText);
		
		if(srch_result.length > 0){
			obj.data = [];
			obj.data = srch_result;
			d.getElementById("userselectortab_sel").style.display = 'none';
		}else{
			obj.data = obj.buffer;
			d.getElementById("userselectortab_sel").style.display = 'block';
		}

		obj.searchi();
	
	}
	req.open("POST", siteurl+"ajax/followusrsearch/r:"+Math.round(Math.random()*1000), true);
	req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	req.send("txt="+encodeURIComponent(txt));
	//added end
...
//added
UserSelector.prototype.searchi	= function()
{
	var usrs	= [];
	var i, j, c, dv, tmp, str, pos;
	for(i=0; i<this.data.length; i++) {
