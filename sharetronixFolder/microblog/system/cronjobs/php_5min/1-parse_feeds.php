<?php
	...
	while($obj = $db2->fetch_object($r)) {
		if( $obj->hub_url!="" && $obj->hub_lastping>time()-12*60*60 ) {
			continue;
		}
	...
	$r	= $db2->query('SELECT ur.* FROM `users_rssfeeds` ur, `users` u WHERE ur.user_id=u.id AND u.active=1 AND ur.is_deleted=0');
	while($obj = $db2->fetch_object($r)) {
		if( $obj->hub_url!="" && $obj->hub_lastping>time()-12*60*60 ) {
			continue;
		}
		...
		if( count($items) > 0 ) {
			$posts	= 0;
			foreach($items as $item) {	
				if(!$obj->is_twitter) $message	= $item->source_title;
					else $message	= preg_replace('/^(.)*:/iusU', '', $item->source_title);
				
				if( empty($message) && !empty($item->source_description) ) {
					if(!$obj->is_twitter) $message	= $item->source_description;
						else $message	= preg_replace('/^(.)*:/iusU', '', $item->source_description);
				}
				if( empty($message) ) {
					continue;
				}
