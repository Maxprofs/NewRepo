<?php
	...
	if( isset($_GET['resend_to']) && !empty($_GET['resend_to']) && in_array(urldecode($_GET['resend_to']), $could_be_invited) ){
		...
		if( $obj && $check && (($obj->date+(3*24*60*60))<time()) ){
			...
			$whom	= $obj->fullname;
			$D->lang_keys	= array('#WHO#'=>$who, '#WHOM#'=>$whom, '#COMPANY#'=>$C->COMPANY, '#SITE_TITLE#'=>$C->SITE_TITLE, '#SITE_URL#'=>$C->SITE_URL);
			...
			if($key && !empty($key)){
				$D->invites[$key]->resend = FALSE;
				$D->invites[$key]->date = $current_time;
			}
		}
