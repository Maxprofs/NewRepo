<?php
	...
	$api_session->available_resources 	= array('follow', 'unfollow', 'membership', 'all_groups', 'new', 'destroy');
	...
	}/*elseif(($api_session->resource == 'follow' || $api_session->resource == 'unfollow') && isset($api_session->resource_option))
	{
		...
		if($api_session->resource == 'follow') $ok = $user->follow_group($group_id);
		elseif($api_session->resource == 'unfollow') $ok = $user->follow_group($group_id, FALSE);
	...
	}*/elseif(($api_session->resource == 'membership'))
	{
		...
		$res = $this->db2->query('SELECT groups_followed.user_id AS user FROM groups_followed, groups, users WHERE groups_followed.user_id=users.id AND groups_followed.group_id=groups.id AND groups_followed.group_id="'.intval($this->db2->e($gid)).'"');
		...
		if($num_rows > 0)
		{
			...
			{				
				$answer .= $twitter_data->data_section('user');		
					$answer .= $twitter_data->data_field('id', $obj->cid, FALSE);		
				$answer .= $twitter_data->data_section('user', FALSE, TRUE);
	...
	}/*elseif($api_session->resource == 'create')
	{
		...
		if( !$error ) {
			if( ! preg_match('/^[a-z0-9\-\_]{3,30}$/iu', $form_groupname) ) {
				...
				if( $res =  $this->db2->fetch_fiels('SELECT id FROM groups WHERE (groupname="'.$this->db2->e($form_groupname).'" OR title="'.$this->db2->e($form_groupname).'") LIMIT 1')) {
					$error	= TRUE;
					$errmsg	= 'Provide different group name.';
				}
				else {
		...
		if( ! $error ) {
			...
			$res = $this->db2->query('SELECT groupname AS gn, group_id AS gi FROM groups, groups_admins WHERE user_id ='.intval($id).' ORDER BY groups_admins.id DESC LIMIT 1');
