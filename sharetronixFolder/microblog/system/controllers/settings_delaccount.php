<?php
	...
	if( isset($_POST['userpass']) ) 
	{
		...
		if( ! $D->error ) 
		{
			$db2->query('DELETE FROM groups_admins WHERE user_id="'.$this->user->id.'"');
			$db2->query('DELETE FROM groups_private_members WHERE user_id="'.$this->user->id.'"');
			$db2->query('DELETE FROM post_favs WHERE user_id="'.$this->user->id.'"');
			$db2->query('DELETE FROM post_userbox WHERE user_id="'.$this->user->id.'"');
			$db2->query('DELETE FROM post_userbox_feeds WHERE user_id="'.$this->user->id.'"');
			$db2->query('DELETE FROM searches WHERE user_id="'.$this->user->id.'"');
			$db2->query('DELETE FROM users_dashboard_tabs WHERE user_id="'.$this->user->id.'"');
			$db2->query('DELETE FROM users_details WHERE user_id="'.$this->user->id.'" LIMIT 1');
			$db2->query('DELETE FROM users_invitations WHERE user_id="'.$this->user->id.'" OR recp_user_id="'.$this->user->id.'"');
			...
			$$db2->query('DELETE FROM notifications WHERE from_user_id="'.$this->user->id.'" OR to_user_id="'.$this->user->id.'"');
			$db2->query('DELETE FROM group_notifications WHERE from_user_id="'.$this->user->id.'"');
			...
			$res	= $db2->query('SELECT id, post_id FROM posts_reshares WHERE user_id="'.$this->user->id.'" ');
			while($tmp = $db2->fetch_object($res)) {
