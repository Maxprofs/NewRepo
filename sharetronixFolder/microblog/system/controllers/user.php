<?php
	...
	if($D->tab == 'updates') {
		...
		if( !$D->posts_protected ){
			...
			if($D->filter == 'all') {
				$q2	= 'SELECT p.*, p.id AS pid, "public" AS `type` FROM posts p WHERE ((p.user_id="'.$u->id.'" '.$not_in_groups.') '.( count($reshared)==0 ? '' : ' OR p.id IN('.implode(', ',$reshared).')' ).') '.$start_from.' ORDER BY p.id DESC ';
			}elseif($D->filter == 'posts'){
				$q2	= 'SELECT p.*, p.id AS pid, "public" AS `type` FROM posts p WHERE p.user_id="'.$u->id.'" '.$not_in_groups.' AND p.api_id<>2 AND p.api_id<>6 '.$start_from.' ORDER BY p.id DESC ';
			}elseif($D->filter == 'tweets'){
				$q2	= 'SELECT p.*, p.id AS pid, "public" AS `type` FROM posts p WHERE p.user_id="'.$u->id.'" '.$not_in_groups.' AND p.api_id=6 '.$start_from.' ORDER BY p.id DESC ';
			}elseif($D->filter == 'rss'){
				$q2	= 'SELECT p.*, p.id AS pid, "public" AS `type` FROM posts p WHERE p.user_id="'.$u->id.'" '.$not_in_groups.' AND p.api_id=2 '.$start_from.' ORDER BY p.id DESC ';
			}elseif( $D->filter == 'reshares' ){
				$q2	= 'SELECT p.*, pr.id AS pid, "public" AS `type` FROM posts p, posts_reshares pr WHERE p.id=pr.post_id AND (pr.user_id="'.$u->id.'" OR p.user_id="'.$u->id.'") '.$start_from.' GROUP BY pr.post_id ORDER BY pr.id ';
			}
	...
	elseif($D->tab == 'coleagues') {
		...
		if($D->show_tab_incommon){
			$D->users_in_common = array_keys(array_intersect_key($he_follows, $i_follow));
