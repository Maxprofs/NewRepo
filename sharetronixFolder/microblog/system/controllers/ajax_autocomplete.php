<?php
	...
	if( $datatype == 'username' )
	{
		...
		$r	= $this->db2->query('SELECT id, username, fullname, avatar FROM users WHERE active=1 AND (username LIKE "'.$w.'%" OR fullname LIKE "'.$w.'%" OR fullname LIKE "% '.$w.'%") ORDER BY num_followers DESC, fullname ASC LIMIT 5');
		$i	= 0;
		$n	= $this->db2->num_rows();
		while($obj = $this->db2->fetch_object($r)) {
			...
			$html	.= '<img src="'.$C->IMG_URL.'avatars/thumbs3/'.(!empty($obj->avatar)? $obj->avatar : $GLOBALS['C']->DEF_AVATAR_USER).'" alt="" />';
			$html	.= '<div><b>'.$fullname.'</b>@'.$username.'</div>';
			$html	.= '</a>';
			echo '<row word="'.htmlspecialchars($obj->username).'" url="'.htmlspecialchars($C->SITE_URL.$obj->username).'" html="'.htmlspecialchars($html).'" />';
		}
	}
	elseif( $datatype == 'groupname' )
	{
		...
		$w	= $this->db2->escape($word);
		$r	= $this->db2->query('SELECT id, title, groupname, about_me FROM groups WHERE (groupname LIKE "%'.$w.'%" OR title LIKE "%'.$w.'%") '.$not_in_groups.' ORDER BY num_followers DESC, title ASC LIMIT 5');
		$i	= 0;
		$n	= $this->db2->num_rows();
		while($obj = $this->db2->fetch_object($r)) {
			$i	++;
			$title	= str_cut($obj->title, 14);
			...
			$html	= '<a href="javascript:;" style="'.($i==$n?'border-bottom:0px;':'').'">';
			$html	.= '<img src="'.$C->IMG_URL.'avatars/thumbs3/'.(!empty($obj->avatar)? $obj->avatar : $GLOBALS['C']->DEF_AVATAR_USER).'" alt="" />';
			$html	.= '<div><b>'.$title.'</b>'.htmlspecialchars(str_cut($obj->about_me,17)).'</div>';
			$html	.= '</a>';
			echo '<row word="'.htmlspecialchars($obj->title).'" url="'.htmlspecialchars($C->SITE_URL.$obj->groupname).'" html="'.htmlspecialchars($html).'" />';
		}
	}
