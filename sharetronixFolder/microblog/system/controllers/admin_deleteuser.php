<?php
	...
	if( isset($_POST['deluser'], $_POST['admpass']) ) {
		...
		elseif( md5($_POST['admpass']) != $this->user->info->password ) {
			...
			$db2->query('DELETE FROM groups_admins WHERE user_id="'.$u->id.'"');
			$db2->query('DELETE FROM groups_private_members WHERE user_id="'.$u->id.'"');
			$db2->query('DELETE FROM post_favs WHERE user_id="'.$u->id.'"');
			$db2->query('DELETE FROM post_userbox WHERE user_id="'.$u->id.'"');
			$db2->query('DELETE FROM post_userbox_feeds WHERE user_id="'.$u->id.'"');
			$db2->query('DELETE FROM post_userbox_tweets WHERE user_id="'.$u->id.'"');
			$db2->query('DELETE FROM searches WHERE user_id="'.$u->id.'"');
			$db2->query('DELETE FROM users_dashboard_tabs WHERE user_id="'.$u->id.'"');
			$db2->query('DELETE FROM users_details WHERE user_id="'.$u->id.'" LIMIT 1');
			$db2->query('DELETE FROM users_invitations WHERE user_id="'.$u->id.'" OR recp_user_id="'.$u->id.'"');
			...
			$db2->query('DELETE FROM notifications WHERE from_user_id="'.$u->id.'" OR to_user_id="'.$u->id.'"');
			$db2->query('DELETE FROM group_notifications WHERE from_user_id="'.$u->id.'"');
			...
			$res	= $db2->query('SELECT id, post_id FROM posts_reshares WHERE user_id="'.$u->id.'" ');
			while($tmp = $db2->fetch_object($res)) {
				$db2->query('DELETE FROM posts_reshares WHERE id="'.$tmp->id.'" LIMIT 1');
				$db2->query('UPDATE posts SET reshares=reshares-1 WHERE id="'.$tmp->post_id.'" LIMIT 1');
			}
	...
	$D->deluser = (isset($_GET['usrtodel']) && !empty($_GET['usrtodel']))? trim(htmlspecialchars(urldecode($_GET['usrtodel']))) : '';
