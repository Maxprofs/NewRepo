<?php
	...
	if( $this->param('my')=='dashboard' ) {
		...
		$q	= 'SELECT p.*, "public" AS `type` FROM post_userbox b LEFT JOIN posts p ON p.id=b.post_id WHERE b.user_id="'.$this->user->id.'" AND (p.api_id NOT IN(2,6) OR p.user_id<>0) ORDER BY b.id DESC LIMIT '.$q_limit;
	}
	...
	elseif( $this->param('my')=='bookmarks' ) {
		...
		$q	= '
			(SELECT p.id, p.api_id, p.user_id, "0" AS comments, "0" AS reshares, p.group_id, "0" AS to_user, p.message, p.mentioned, p.attached, p.posttags, p.date, p.ip_addr, "0" AS is_recp_del, "public" AS `type`, f.id AS fid FROM post_favs f LEFT JOIN posts p ON p.id=f.post_id WHERE f.user_id="'.$this->user->id.'" AND f.post_type="public")
			UNION
			(SELECT p.id, p.api_id, p.user_id, "0" AS comments, "0" AS reshares, "0" AS group_id, p.to_user, p.message, p.mentioned, p.attached, p.posttags, p.date, p.ip_addr, p.is_recp_del, "private" AS `type`, f.id AS fid FROM post_favs f LEFT JOIN posts_pr p ON p.id=f.post_id WHERE f.user_id="'.$this->user->id.'" AND f.post_type="private")
	...
	elseif( $this->param('username') ) {
		...
		if( $u ) {
			...
			$D->he_follows_me = ($this->user->is_logged)? isset($this->network->get_user_follows($u->id, FALSE, 'hefollows')->follow_users[$this->user->id]) : FALSE;
		
			if(!$u->is_posts_protected || $D->he_follows_me || ($this->user->is_logged && $this->user->info->is_network_admin)){
	...
	elseif( $this->param('groupname') ) {
		...
		if( $g ) {
			...
			if( $has_access ) {
				$q	= 'SELECT *, "public" AS `type` FROM posts WHERE group_id="'.$g->id.'" AND (api_id NOT IN(2, 6) OR user_id<>0) ORDER BY id DESC LIMIT '.$q_limit;
			}
		}
	}
