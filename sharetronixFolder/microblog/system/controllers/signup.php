<?php
	...
	if( $C->USERS_EMAIL_CONFIRMATION && isset($_POST['email']) && !empty($_POST['email']) && !$this->param('regid') && !$this->param('regkey'))
	{
		$D->submit	= TRUE;
		$D->error	= FALSE;
		$D->errmsg	= '';
	...
	{
                $D->email =''; $D->email_status = FALSE;
                $D->fullname ='';$D->fullname_status =FALSE;
                $D->username ='';$D->username_status =FALSE;
                $D->password2	= isset($_POST['password2'])?(trim($_POST['password2'])):'';
                $D->password ='';$D->password_status =FALSE;
                if(isset($_POST['fullname'],$_POST['email']) && !isset($_POST['username'])){
                    $_POST['username']='';
                }
                if(isset($_POST['password'])){
                    $D->password	= trim($_POST['password']);
                    if(!empty($D->password) && empty($D->password2)){
                        $D->password_status = '<div id="Error">' . $this->lang('signup_step2_form_password2') . '</div>';
                    }
                    if(!empty($D->password) && !empty($D->password2) && ($D->password !=$D->password2)){
                         $D->password_status = '<div id="Error">' . $this->lang('signup_err_passwddiff') . '</div>';
                    }
                    if(!empty($D->password) && !empty($D->password2) && ($D->password ==$D->password2)){
                         $D->password_status = '<div id="Success">' . $this->lang('signup_register_password_ok') . '</div>';
                    }
                }
		if( $twinfo && isset($twinfo->tmp_valid) && $twinfo->tmp_valid ) {
			$D->email	= '';
			$D->fullname	= $twinfo->name;
			$_POST['fullname'] = $D->fullname;
			$_POST['username'] = $twinfo->screen_name;
			$_POST['email'] = $D->email;
		}
		if( $fbinfo && isset($fbinfo->tmp_valid) && $fbinfo->tmp_valid ) {
			...
			$_POST['fullname'] = $D->fullname;
			$_POST['username'] = $fbinfo->username;
			$_POST['email'] = $D->email;
		}
		$invited_code	= '';
		if( $C->USERS_EMAIL_CONFIRMATION ) {
			...
			$D->email		= stripslashes($obj->email);
                        $D->email_status = '<div id="Success">'. $this->lang('signup_register_email_ok') . '</div>';
                        $D->reg_id = $reg_id;
                        $D->reg_key = $reg_key;
		...
		if( isset($_POST['fullname'], $_POST['username']) ) {
				...
				if( empty($D->email) ){
					$D->email	= isset($_POST['email']) ? strtolower(trim($_POST['email'])) : '';
                                ...
                                if( ! is_valid_email($D->email) ) {
                                    $D->error	= TRUE;
                                    $D->errmsg	= 'signup_err_email_invalid';
                                    $D->email_status = '<div id="Error">' . $this->lang($D->errmsg) . '</div>';
                                } else {
			...
			if( !$D->username_status && empty($D->username) ) {
				$D->error	= TRUE;
				$D->errmsg	= 'signup_err_username';
                                $D->username_status ='<div id="Error">' . $this->lang($D->errmsg) . '</div>';
			}
			if( !$D->username_status && (strlen($D->username)<3 || strlen($D->username)>30) ) {
				$D->error	= TRUE;
				$D->errmsg	= 'signup_err_usernmlen';
                                $D->username_status ='<div id="Error">' . $this->lang($D->errmsg) . '</div>';
			}
			if( !$D->username_status && preg_match('/[^a-z0-9-_]/i', $D->username) ) {
				$D->error	= TRUE;
				$D->errmsg	= 'signup_err_usernmlet';
                                $D->username_status ='<div id="Error">' . $this->lang($D->errmsg) . '</div>';
			}
			if( !$D->username_status ) {
			...
			if( !$D->username_status && file_exists($C->INCPATH.'controllers/'.strtolower($D->username).'.php') ) {
				$D->error	= TRUE;
				$D->errmsg	= 'signup_err_usernm_existss';
                                $D->username_status ='<div id="Error">' . $this->lang($D->errmsg) . '</div>';
			}
			if( !$D->username_status && file_exists($C->INCPATH.'controllers/mobile/'.strtolower($D->username).'.php') ) {
				$D->error	= TRUE;
				$D->errmsg	= 'signup_err_usernm_existss';
                                $D->username_status ='<div id="Error">' . $this->lang($D->errmsg) . '</div>';
			}
			if( !$D->username_status && file_exists($C->INCPATH.'../'.strtolower($D->username)) ) {
				$D->error	= TRUE;
				$D->errmsg	= 'signup_err_usernm_existss';
                                $D->username_status ='<div id="Error">' . $this->lang($D->errmsg) . '</div>';
			}
                        ($D->username_status)?$D->username_status:$D->username_status = '<div id="Success">'. $this->lang('signup_register_username_ok') . '</div>';
				...
				if( isset($C->GOOGLE_CAPTCHA_PRIVATE_KEY, $C->GOOGLE_CAPTCHA_PUBLIC_KEY) && $C->GOOGLE_CAPTCHA_PRIVATE_KEY == '' && $C->GOOGLE_CAPTCHA_PUBLIC_KEY == '' ){
					if( !isset($_POST['captcha_key'],$_POST['captcha_word']) || !isset($_SESSION['captcha_'.$_POST['captcha_key']]) || $_SESSION['captcha_'.$_POST['captcha_key']]!=strtolower($_POST['captcha_word']) ) {
						...
						$D->errmsg	= 'signup_err_captcha';
                                                $D->captcha_status = '<div id="Error">' . $this->lang('signup_err_captcha') . '</div>';
					}
				}else{
			...
			if( !$D->error ) {
                ...
                if(!empty($D->fullname)){
                    if(strlen($D->fullname) <=2 && strlen($D->fullname) >0){
                        $D->fullname_status = '<div id="Error">' . $this->lang('signup_register_fullname_notreal') . '</div>';
                    }else {
                        $D->fullname_status = '<div id="Success">' . $this->lang('signup_register_fullname_ok') . '</div>';
                    }
                }
		$this->load_template('signup1.php');
	}
