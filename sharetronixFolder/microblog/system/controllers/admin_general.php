<?php
	...
	$D->pages_protect = intval($s->PROTECT_OUTSIDE_PAGES);
	$D->twitter_tag	= ($s->POST_FROM_TWITTER_TAG != '0')? $s->POST_FROM_TWITTER_TAG:'';
	$D->api_status	= intval($s->API_STATUS);
	$D->check4update	= isset($s->AUTOCHECK_FOR_UPDATE) && $s->AUTOCHECK_FOR_UPDATE==0 ? FALSE : TRUE;
	...
	if( isset($_POST['sbm']) ) {
		...
		$D->pages_protect		= isset($_POST['pages_protect']) && $_POST['pages_protect']==1;
		$D->api_status		= isset($_POST['api_status']) && $_POST['api_status']==1;
		$D->check4update		= isset($_POST['check4update']) && $_POST['check4update']==1;
		...
		if( isset($_POST['twitter_tag']) ) {
			$D->twitter_tag	= preg_replace('/#/iu', '', $_POST['twitter_tag']);
		}
		...
		$db2->query('REPLACE INTO settings SET word="PROTECT_OUTSIDE_PAGES", value="'.intval($D->pages_protect).'" ');
		$db2->query('REPLACE INTO settings SET word="API_STATUS", value="'.intval($D->api_status).'" ');
		$db2->query('REPLACE INTO settings SET word="POST_FROM_TWITTER_TAG", value="'.$D->twitter_tag.'" ');
