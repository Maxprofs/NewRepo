<?php
	...
	$this->load_langfile('mobile/signup.php');
	$this->load_langfile('outside/signup.php');
	
	$D->page_title	= $this->lang('m_site_title', array('#SITE_TITLE#'=>$C->SITE_TITLE));
	...
	elseif( ($C->USERS_EMAIL_CONFIRMATION && $this->param('regid') && $this->param('regkey')) || !$C->USERS_EMAIL_CONFIRMATION )
	{
		...
		if( isset($_POST['fullname'], $_POST['username']) ) {
			...
			if( !$D->error && file_exists($C->INCPATH.'controllers/mobile_iphone/'.strtolower($D->username).'.php') ) {
				$D->error	= TRUE;
				$D->errmsg	= 'signup_err_usernm_existss';
			}
		...
		$this->load_template('mobile_iphone/signup-step2.php');
	}
