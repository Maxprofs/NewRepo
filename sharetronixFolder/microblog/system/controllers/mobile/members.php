<?php
	...
	elseif( $D->show == 'following' ) {
		$members = $this->network->get_user_follows($this->user->id, FALSE, 'hefollows')->follow_users;
		$D->num_results	= count($members);
		$D->num_pages	= ceil($D->num_results / $C->PAGING_NUM_USERS);
		$D->pg	= min($D->pg, $D->num_pages);
		$D->pg	= max($D->pg, 1);
		$from	= ($D->pg - 1) * $C->PAGING_NUM_USERS;
		$tmp	= array_keys(array_slice($members, $from, $C->PAGING_NUM_USERS, TRUE));
		unset($members);
	}
	elseif( $D->show == 'followers' ) {
		$members = $this->network->get_user_follows($this->user->id, FALSE, 'hisfollowers')->followers;
		$D->num_results	= count($members);
		$D->num_pages	= ceil($D->num_results / $C->PAGING_NUM_USERS);
		$D->pg	= min($D->pg, $D->num_pages);
		$D->pg	= max($D->pg, 1);
