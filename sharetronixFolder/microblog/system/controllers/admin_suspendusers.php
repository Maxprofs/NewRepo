<?php
	...
	if( isset($_POST['admins']) ) {
		...
		if( count($ids) ) {
			...
			$this->db2->query('UPDATE users_rssfeeds SET is_deleted=1 WHERE user_id IN('.implode(', ', $ids).') ');
		}
		foreach($ids as $a) {
	...
	$D->usrtosusp = (isset($_GET['usrtosusp']) && !empty($_GET['usrtosusp']))? trim(htmlspecialchars(urldecode($_GET['usrtosusp']))) : '';
