<?php
	...
	class rssfeed
	{
		...
		public function __construct($feed_url, $feed_auth_userpwd='')
		{
			...
			$this->posts_number_filter = $C->RSS_FEED_POSTS_NUMBER_FILTER;
		}
		...
		public function fetch()
		{
			...
			if( preg_match('/\<link.*rel\=(\"|\\\')hub(\"|\\\').*\>/iuU', $this->raw, $m) ) {
				if( preg_match('/href\=(\"|\\\')(.*)(\"|\\\')/iuU', $m[0], $m) ) {
					$this->hub	= trim($m[2]);
				}
			}
			...
			if( $this->posts_number_filter ){
				$this->filter_fetched_posts_number();
			}
		...
		public function filter_fetched_posts_number()
		{
			if( !$this->posts_number_filter ){
				return TRUE;
			}
			
			$last_date = $this->get_lastitem_date();
			$last_item = FALSE;
			foreach( $this->items as $itm ){
				if( $itm->source_date == $last_date ){
					$last_item = $itm;
				}
			}
