<?php
	...
	class network
	{
		...
		public function get_user_by_twitter_username($uname, $force_refresh=FALSE, $return_id=FALSE)
		{
			if( ! $this->id ) {
				return FALSE;
			}
		...
		public function get_user_by_email($email, $force_refresh=FALSE, $return_id=FALSE)
		{
			...
			$uid	= FALSE;
			$r	= $this->db2->query('SELECT id FROM users WHERE email="'.$this->db2->escape($email).'" LIMIT 1', FALSE);
			if( $o = $this->db2->fetch_object($r) ) {
				$uid	= intval($o->id);
				$this->cache->set($cachekey, $uid, $GLOBALS['C']->CACHE_EXPIRE);
				return $return_id ? $uid : $this->get_user_by_id($uid);
		...
		public function get_mostactive_users($force_refresh=FALSE)
		{
			...
			$data	= array();
			$this->db2->query('SELECT user_id, COUNT(*) AS c FROM posts WHERE user_id<>0 AND api_id<>2 AND api_id<>6 GROUP BY user_id ORDER BY c DESC LIMIT 10');
		...
		public function get_mostcommenting_users($force_refresh=FALSE)
		{
			...
			$data	= array();
			$this->db2->query('SELECT user_id, COUNT(*) AS c FROM posts_comments WHERE user_id<>0 GROUP BY user_id ORDER BY c DESC LIMIT 10');
			while($obj = $this->db2->fetch_object()) {
				$usr = $this->get_user_by_id($obj->user_id);
				if( !$usr ){
					continue;
		...
		public function get_mostcommented_users($force_refresh=FALSE)
		{
			...
			$data	= array();
			$this->db2->query('SELECT posts.user_id AS uid, COUNT(posts_comments.id) AS c FROM posts, posts_comments WHERE posts.user_id=posts_comments.user_id AND posts_comments.post_id=posts.id AND posts.user_id<>0 AND posts_comments.user_id<>0 GROUP BY posts.user_id ORDER BY c DESC LIMIT 10');
			while($obj = $this->db2->fetch_object()) {
		...
		public function get_mostfollowed_users($force_refresh=FALSE)
		{
			...
			$data	= array();
			$this->db2->query('SELECT id, username, avatar, num_followers AS c FROM users WHERE num_followers > 0 ORDER BY num_followers DESC LIMIT 10');
			while($obj = $this->db2->fetch_object()) {
				if( empty($obj->avatar) ){
		...
		public function get_mostfollowing_users($force_refresh=FALSE)
		{
			...
			$data	= array();
			$this->db2->query('SELECT who, COUNT(*) AS c FROM `users_followed` GROUP BY who ORDER BY `c` DESC LIMIT 10');
			while($obj = $this->db2->fetch_object()) {
